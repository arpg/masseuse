...
public:
    ...
    if( !m_vPath.empty() ) {
      ...
      if( m_bDrawAxis ) {
        ...
        for( size_t ii = 0; ii < m_vPath.size(); ++ii ) {
          ...
          bool plot_pose =
              (m_nPoseDisplay == 0 && m_nPoseDisplayFrequency == 0) ||
              (m_nPoseDisplay != 0 && static_cast<int>(ii) >= start) ||
              (m_nPoseDisplayFrequency != 0 && static_cast<int>(ii)%m_nPoseDisplayFrequency == 0);
          if (plot_pose) {
  ...
  void SetPoseDisplayFrequency( unsigned int Num )
  {
    m_nPoseDisplayFrequency = Num;
  }
...
private:
  ...
  unsigned int                    m_nPoseDisplayFrequency;
