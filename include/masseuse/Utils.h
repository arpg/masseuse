...
static Eigen::IOFormat kLongCsvFmtNoLineBreak(Eigen::FullPrecision, 0, ", ", "", "", "");
...
inline T powiJet(const T x, const int y) {
  if (y == 0) {
    return T(1.0);
  } else if (y < 0) {
    return T(1.0) / powiJet<T, Scalar>(x, -y);
  } else if (y == 0) {
    return T(1.0);
  } else {
    T ret = x;
...
// this function implements the derivative of log with respect to the input q
template<typename T, typename Scalar = double>
inline Eigen::Matrix<T, 3, 4> dlog_dqJet(const Eigen::Quaternion<T>& q) {
  const T x = q.x();
  const T y = q.y();
  const T z = q.z();
  const T w = q.w();
  const T vec_squarednorm = powiJet<T, Scalar>(x, 2) +
      powiJet<T, Scalar>(y, 2) + powiJet<T, Scalar>(z, 2);
  const T vec_norm = sqrt(vec_squarednorm);
  if (vec_norm < 1e-9) {
    // StreamMessage(debug_level) << "Vec norm less than 1e-9: " <<
    //                               q.coeffs().transpose() << std::endl;
    const T s1 = T(2) * vec_squarednorm;
    const T s2 = T(1.0) / powiJet<T, Scalar>(w, 3);
    const T s3 = (T(3) * s1) / powiJet<T, Scalar>(w, 4) -
        T(2) / powiJet<T, Scalar>(w, 2);
    const T s4 = T(2) / w;

    return (Eigen::Matrix<T, 3, 4>()
            << T(-4) * s2 * powiJet<T, Scalar>(x, 2) + s4 - s1 * s2,
            T(-4) * x * y * s2,
            T(-4) * x * z * s2,
            x * s3,
            T(-4) * x * y * s2,
            T(-4) * s2 * powiJet<T, Scalar>(y, 2) + s4 - s1 * s2,
            T(-4) * y * z * s2,
            y * s3,
            T(-4) * x * z * s2,
            T(-4) * y * z * s2,
            T(-4) * s2 * powiJet<T, Scalar>(z, 2) + s4 - s1 * s2,
            z * s3).finished();

  } else {
    const T s1 = vec_squarednorm;
    const T s2 = T(1) / (s1 / powiJet<T, Scalar>(w, 2) + T(1));
    const T s3 = ceres::atan(sqrt(s1) / w);
    const T s4 = T(1) / ceres::pow(s1, T(3.0 / 2.0));
    const T s5 = T(1) / s1;
    const T s6 = T(1) / w;
    const T s7 = (T(2) * s3) / sqrt(s1);
    const T s8 = T(2) * y * z * s2 * s5 * s6 - T(2) * y * z * s3 * s4;
    const T s9 = T(2) * x * z * s2 * s5 * s6 - T(2) * x * z * s3 * s4;
    const T s10 = T(2) * x * y * s2 * s5 * s6 - T(2) * x * y * s3 * s4;

    return (Eigen::Matrix<T, 3, 4>()
            << s7 - T(2) * powiJet<T, Scalar>(x, 2) * s3 * s4 + T(2) *
            powiJet<T, Scalar>(x, 2) * s2 * s5 * s6, s10,
            s9, -(T(2)* x * s2) / powiJet<T, Scalar>(w, 2), s10, s7
            - T(2) * powiJet<T, Scalar>(y, 2) * s3 * s4
            + T(2) * powiJet<T, Scalar>(y, 2) * s2 * s5 * s6, s8,
            -(T(2) * y * s2) / powiJet<T, Scalar>(w, 2), s9, s8,
            s7 - T(2) * powiJet<T, Scalar>(z, 2) * s3 * s4 + T(2) *
            powiJet<T, Scalar>(z, 2) * s2 * s5 * s6,
            -(T(2) * z * s2) / powiJet<T, Scalar>(w, 2)).finished();
  }
}
...
///////////////////////////////////////////////////////////////////////////////
template<typename T, typename Scalar = double>
inline Eigen::Matrix<T, 4, 3> dq_exp_dwJet(
    const Eigen::Matrix<T, 3, 1>& w) {
  const T t = T(w.norm());
  const T s1 = t / T(20 - 1);
  const T s2 = powiJet<T, Scalar>(t, 2) / T(48 - 0.5);
  const T s3 = (s1 * w[1] * w[2]) / T(24);
  const T s4 = (s1 * w[0] * w[2]) / T(24);
  const T s5 = (s1 * w[0] * w[1]) / T(24);
  const T s6 = powiJet<T, Scalar>(t, 2);
  return (Eigen::Matrix<T, 4, 3>()
          << (s1 * powiJet<T, Scalar>(w[0], 2)) / T(24) - s6 / T(48) + T(0.5),
      s5, s4, s5, (s1
                   * powiJet<T, Scalar>(w[1], 2)) / T(24) - s6 / T(48) + T(0.5), s3, s4,
      s3, (s1 * powiJet<T, Scalar>(w[2], 2))
      / T(24) - s6 / T(48) + T(0.5), (s2 * w[0]) / T(2),
      (s2 * w[1]) / T(2), (s2 * w[2]) / T(2))
      .finished();
}
...
///////////////////////////////////////////////////////////////////////////////
template<typename T, typename Scalar = double>
inline Eigen::Matrix<T, 4, 4> dq1q2_dq2Jet(
    const Eigen::Quaternion<T>& q1) {
  return (Eigen::Matrix<T, 4, 4>() << q1.w(), -q1.z(), q1.y(), q1.x(),
          q1.z(), q1.w(), -q1.x(), q1.y(), -q1.y(), q1.x(), q1.w(), q1.z(),
          -q1.x(), -q1.y(), -q1.z(), q1.w()).finished();
}
...
///////////////////////////////////////////////////////////////////////////////
template<typename T, typename Scalar = double>
inline Eigen::Matrix<T, 4, 4> dq1q2_dq1Jet(
    const Eigen::Quaternion<T>& q2) {
  return (Eigen::Matrix<T, 4, 4>() << q2.w(), q2.z(), -q2.y(), q2.x(),
          -q2.z(), q2.w(), q2.x(), q2.y(), q2.y(), -q2.x(), q2.w(), q2.z(),
          -q2.x(), -q2.y(), -q2.z(), q2.w()).finished();
}
...
inline Eigen::Matrix<Scalar, 3, 4> dqx_dq(
  ...
  Eigen::Matrix<Scalar, 3,4> dqx_dq =
      (Eigen::Matrix<Scalar, 3, 4>() << s2 + s6, s1 - 4 * q.y() * x
  ...
  // Check the dqx_dq
//  {
//    Eigen::Matrix<double, 3, 4>  dqx_dq_fd;
//    Scalar deps = 1e-6;
//    for(int ii = 0; ii < 4 ; ++ii){
//        Eigen::Matrix<double, 4, 1> eps_vec;
//        eps_vec.setZero();
//        eps_vec[ii] = deps;

//        // Note: the quaternion cannot be normalized, as that will change
//        // the other parameters
//        Eigen::Quaterniond q_plus = q;
//        q_plus.coeffs() += eps_vec;

//        Eigen::Matrix<double, 3, 1> y_plus = q_plus*vec;

//        eps_vec[ii] = -deps;
//        Eigen::Quaterniond q_minus = q;
//        q_minus.coeffs() += eps_vec;

//        Eigen::Matrix<double, 3, 1> y_minus = q_minus * vec;

//        dqx_dq_fd.col(ii) = (y_plus - y_minus) / (2 * deps);
//    }
//    std::cerr << "dqx_dq:" << dqx_dq.format(kCleanFmt) << std::endl;
//    std::cerr << "dqx_dq_fd:" << dqx_dq_fd.format(kCleanFmt) << std::endl;
//    std::cerr << "norm: " << (dqx_dq - dqx_dq_fd).norm() << std::endl;
//  }
...
///////////////////////////////////////////////////////////////////////////////
template<typename T, typename Scalar = double>
inline Eigen::Matrix<T, 3, 4> dqx_dqJet(
    const Eigen::Quaternion<T>& q,
    const Eigen::Matrix<T, 3, 1>& vec) {
  const T x = vec[0], y = vec[1], z = vec[2];
  const T s1 = T(2) * q.x() * y;
  const T s2 = T(2) * q.y() * y;
  const T s3 = T(2) * q.x() * x;
  const T s4 = T(2) * q.z() * x;
  const T s5 = T(2) * q.y() * z;
  const T s6 = T(2) * q.z() * z;

  return (Eigen::Matrix<T, 3, 4>() << s2 + s6, s1 - T(4) * q.y() * x
          + T(2) * q.w() * z, T(2) * q.x() * z - T(2) * q.w() * y - T(4) * q.z() * x, s5
          - T(2) * q.z() * y, T(2) * q.y() * x - T(4) * q.x() * y - T(2) * q.w() * z, s3 + s6,
          s5 + T(2) * q.w() * x - T(4) * q.z() * y, s4 - T(2) * q.x() * z, s4 + T(2) * q.w() * y
          - T(4) * q.x() * z, T(2) * q.z() * y - T(2) * q.w() * x - T(4) * q.y() * z, s2 + s3,
          s1 - T(2) * q.y() * x).finished();
}
...
///////////////////////////////////////////////////////////////////////////////
template<typename Scalar = double>
inline Eigen::Matrix<Scalar, 6, 7> dLog_decoupled_dt1Ceres(
    const Sophus::SE3Group<Scalar>& t1, const Sophus::SE3Group<Scalar>& t2) {
  Eigen::Matrix<Scalar, 6, 7> dLog_decoupled;
  dLog_decoupled.setZero();
  dLog_decoupled.template block<3, 3>(0, 4).setIdentity();
  dLog_decoupled.template block<3, 4>(3, 0) =
      dlog_dq((t1*t2.inverse()).unit_quaternion()) *
      dq1q2_dq1(t2.inverse().unit_quaternion());
  return dLog_decoupled;
}
...
///////////////////////////////////////////////////////////////////////////////
template<typename T, typename Scalar = double>
inline Eigen::Matrix<T, 6, 7> dLog_decoupled_dt1Jet(
    const Sophus::SE3Group<T>& t1, const Sophus::SE3Group<T>& t2) {
  Eigen::Matrix<T, 6, 7> dLog_decoupled;
  dLog_decoupled.setZero();
  dLog_decoupled.template block<3, 3>(0, 0).setIdentity();
  dLog_decoupled.template block<3, 4>(3, 3) =
      dlog_dqJet<T, Scalar>((t1*t2.inverse()).unit_quaternion()) *
      dq1q2_dq1Jet<T, Scalar>(t2.inverse().unit_quaternion());
  return dLog_decoupled;
}
...
///////////////////////////////////////////////////////////////////////////////
template<typename Scalar = double>
inline Eigen::Matrix<Scalar, 7, 7> dx_decoupled_dx(
    const Sophus::SE3Group<Scalar>& t) {
  Eigen::Matrix<Scalar, 7, 7> dexp;
  dexp.setZero();
  dexp.template block<3, 3>(0, 0).setIdentity();
  dexp.template block<4, 4>(3, 3) =
